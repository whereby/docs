...
A hostRoomURL is only available if the `hostRoomURL` is included in the `fields` parameter when [creating a meeting via the API](whereby-rest-api-reference/#create-meeting).
...
Now that you have both a `roomUrl` and a `hostRoomUrl`, you can ensure that both your guests and hosts get the right privileges by using the corresponding URL when [embedding rooms](tbd-archive/embedding-rooms/) in your own webpage or app.
...
A `roomUrl` will be included with any POST [meeting creation](whereby-rest-api-reference/#create-meeting) requests.
...
Explore more in the [API reference](whereby-rest-api-reference/).
