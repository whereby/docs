...
description: >-
  ...
  plan. This page describes how to set up HIPAA compliant video calls.
---
...
{% hint style="warning" %}
Any feature that uses **Whereby-provided storage** is not considered to be HIPAA-compliant. For example, to be HIPAA-compliant with Cloud Recordings, you must store them using an S3 storage bucket.
{% endhint %}
...
The following information describes what it means to use the HIPAA compliant video conferencing capabilities of Whereby. Follow the steps below to get started:
...
{% hint style="success" %}
**Update:** Whereby is proud to now offer HIPAA compliant [cloud recording](../whereby-product-features/recording-with-embedded/cloud-recording.md), in addition to our previously available HIPAA compliant [local recording](../whereby-product-features/recording-with-embedded/local-recording.md). To remain HIPAA compliant while using cloud recording, you need to store the recordings in an S3 storage managed by your organization.
{% endhint %}
...
Please note that key rotation will provide additional security, but also could impact operations, like receiving a permissions error when trying to record/transcribe.\
\
Instead, we request that you create a bucket with a specific and strict [bucket policy](https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucket-policies.html) to ensure compliance:
...
[Local recording setup and instruction](../whereby-product-features/recording-with-embedded/local-recording.md#enabling-local-recording)
...
To do either of the above, you can refer to the "[Using URL parameters](../whereby-for-web-browser/web-component-and-pre-built-ui/configuring-with-attributes.md)" section of the Whereby Developer documentation and use the URL parameter `?displayName=` &#x20;

### Enable privacy toggle&#x20;

Enable privacy settings globally across all rooms created on your account to remove the display name from all areas of Whereby, not just in the meeting room. This can be found in the [Privacy Features](../whereby-product-features/dashboard-preferences/#privacy-features) section of your dashboard. &#x20;
...
[Session Transcriptions](../whereby-product-features/transcribing/session-transcription.md) is designed to protect ePHI (Electronic protected health information) and supports compliance with the HIPAA Security Rule. To remain HIPAA compliant while using Session Transcriptions, you need to store the transcriptions in an S3 storage bucket with a strict access policy managed by your organization.

### Live Captions

[Live Captions](../whereby-product-features/live-captions.md) is designed to protect ePHI (Electronic protected health information) and supports compliance with the HIPAA Security Rule.&#x20;

### Encryption in transit&#x20;

There is no action required to enable encryption in transit as it is already enabled and available for all customers, regardless of whether they are using the Whereby Embedded HIPAA compliant package or not. This is by default and through enforcement of our infrastructure's encryption capabilities. This can be seen from our [A+ grading on SSL Labs](https://www.ssllabs.com/ssltest/analyze.html?d=whereby.com\&latest).
...
To support the HIPAA compliance of our customers, we will gladly provide our ISO27001 certificate and our HIPAA compliance checklist, which documents how Whereby complies with specific HIPAA rules. We perform an annual audit through a third-party consultant. For further information, [reach out to your Whereby contact](https://whereby.com/information/contact-sales/).
...
* **Integrations** - We have integrations built with Miro and YouTube that can be used in Whereby meeting rooms. Because these providers are not HIPAA compliant, you must make sure to disable integrations with [roomIntegrations=off](../whereby-for-web-browser/web-component-and-pre-built-ui/configuring-with-attributes.md#roomintegrations) unless you're using another parameter that hides them like ?minimal.
* **Any feature that uses Whereby-provided storage** - For example, Cloud Recording using Whereby-provided storage is not considered to be HIPAA-compliant. To be HIPAA-compliant, you must store your cloud recordings using an S3 storage bucket.
