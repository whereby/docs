...
* Start and stop [recordings](recording-with-embedded/), if available.
* Start and stop [breakout groups](breakout-groups-with-embedded.md), if available.
...
Now that you have both a `roomUrl` and a `hostRoomUrl`, you can ensure that both your guests and hosts get the right privileges by using the corresponding URL when [embedding rooms](../getting-started/overview-of-whereby-embedded/) in your own webpage or app.

### Participants

Users joining via the standard `roomUrl` will have a more typical meeting experience. They will only have access to the features you've set via [URL parameters](../whereby-for-web-browser/web-component-and-pre-built-ui/configuring-with-attributes.md) or the [dashboard](dashboard-preferences/), and they will need to knock to gain entry to locked rooms.
