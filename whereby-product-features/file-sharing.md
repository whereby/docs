...
{% hint style="warning" %}
Files are available for download for the duration of the session _only_. All files are encrypted and securely stored in Whereby-managed cloud storage, and are **automatically deleted** within 1 minute from the end of the session. They are not backed up and cannot be retrieved after the session.
{% endhint %}
...
{% hint style="success" %}
Chat _must be_ enabled for file sharing to work. You can enable chat for all rooms on the Dashboard or per room.
{% endhint %}
...
You can override global configuration for individual rooms via the API during a [room creation](../reference/whereby-rest-api-reference/meetings.md#post-meetings) request or with an [attribute or parameter](../whereby-for-web-browser/web-component-and-pre-built-ui/configuring-with-attributes.md) of `fileSharing=<on|off>`.

## Additional Information

### HIPAA compliance

File sharing is considered to be [HIPAA compliant](../telehealth-industry-solutions/hipaa-compliant-setup.md), as files are securely stored and only available to the participants for the duration of the session.&#x20;
...
2. File sharing is available in both room modes - P2P rooms called 'normal' and SFU rooms called 'group'. However, please note that in both room modes, the files being shared are temporarily stored in Whereby-managed cloud storage. [Learn more about P2P and SFU video sessions.](https://whereby.com/blog/p2p-vs-sfu-video-calls-which-is-best/)
