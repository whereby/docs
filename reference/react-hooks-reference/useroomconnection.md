...
When a participant provides a valid "host" `roomKey` in the [`RoomConnectionOptions`](types.md#roomconnectionoptions) when the [useRoomConnection](useroomconnection.md) hook was created, they will have access to a number of addition host-only events in rooms:
